...
class JSONExporter:
    ...
    def __init__(self, output_base: str = None):
        ...
        Args:
            output_base: Ruta base de salida (None usa OUTPUT_PATH env var)
        """
        ...
        # Crear directorios
        for path in [self.summary_path, self.analytics_path, 
                     self.advanced_path, self.metadata_path]:
    ...
    def _spark_to_dict_list(self, df: DataFrame, limit: int = None) -> List[Dict]:
        ...
        # Manejar tipos de datos especiales (datetime, numpy types)
        for col in pdf.columns:
            if pdf[col].dtype == 'object':
                pdf[col] = pdf[col].astype(str)
            elif 'datetime' in str(pdf[col].dtype):
                pdf[col] = pdf[col].astype(str)
        
        return pdf.to_dict('records')

    def export_summary_metrics(self, metrics: Dict[str, Any], filename: str = 'metrics.json'):
        """
    ...
    def export_time_series(self, df: DataFrame, series_name: str, 
                          date_col: str = 'date', value_col: str = 'count'):
        """
    ...
    def export_distribution(self, df: DataFrame, distribution_name: str):
        ...
        """
        filepath = os.path.join(self.analytics_path, f'{distribution_name}_distribution.json')
    ...
    def export_correlation_matrix(self, correlation_data: Dict[str, Any], 
    ...
    def export_clustering_results(self, clusters_df: DataFrame, 
        ...
        clusters_data = self._spark_to_dict_list(clusters_df, limit=1000)  # Limitar para performance
    ...
    def export_recommendations(self, recommendations_data: Dict[str, Any],
    ...
    def export_execution_metadata(self, dag_id: str, task_id: str, 
